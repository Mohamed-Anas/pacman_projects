...
class ReflexAgent(Agent):
    ...
    def evaluationFunction(self, currentGameState, action):
		...
		"*** YOUR CODE HERE ***"
		ghosts = [ghost.getPosition() for ghost in newGhostStates]
		#trying to get away from the nearest ghost
		value_min = 10000000
		for ghost in ghosts:
			value_min = min(value_min,sqrt((ghost[0]-newPos[0])**2+(ghost[1]-newPos[1])**2))
		closest_ghost_distance = value_min
		#find closes food distance
		newFoodList = newFood.asList()
		value_min_food = 1000000
		oldFoodList = currentGameState.getFood().asList()
		if newPos in oldFoodList:
			return 1000000
		for food in newFoodList:
			value_min_food = min(value_min_food,sqrt((food[0]-newPos[0])**2+(food[1]-newPos[1])**2))
			closest_food_distance = value_min_food
		#cap = 1/float(closest_food_distance) - 1/float(closest_ghost_distance)
		return closest_ghost_distance-closest_food_distance
...
class MinimaxAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState):
		...
		#getting the game state after those moves
		nextStates = [gameState.generateSuccessor(0,action) for action in Moves]

		#getting scores from minimizer
...
class ExpectimaxAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState):
		...
		scores = [self.expecti(0,sate,1) for state in resultStates]
		bestScore = max(scores)
		bestIndices = [index for index in range(len(scores)) if scores[index]==bestScore]
